...
def run_gap_analysis(client_data: dict, competitor_data: dict, model: str = DEFAULT_MODEL) -> dict:
    ...
    # Cap list sizes — depth 5 crawls produce 500+ topics
    # Sort by length (shorter = more likely to be real headings, not nav noise)
    client_sorted   = sorted(set(client_topics),   key=len)[:50]
    comp_sorted     = sorted(set(competitor_topics), key=len)[:80]
    client_sample   = "\n".join(f"- {t}" for t in client_sorted)
    comp_sample     = "\n".join(f"- {t}" for t in comp_sorted)
